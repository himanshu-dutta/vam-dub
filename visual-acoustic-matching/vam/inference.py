...
def main():
    ...
    if os.path.exists(args.from_pretrained):
        ...
        print("Warning: no pretrained model weights are found!: ", args.from_pretrained)
    model.to(device=torch.device("cuda"))
    model.eval()
    with torch.no_grad():
        rgb = np.array(Image.open(args.input_image).convert("RGB"))[:, :, :3]
        audio, _ = librosa.load(args.input_audio, sr=16000)
        model_audio_length = 40960
        print("audio ka shape: ", len(audio))

        audios = list()
        for idx in range(int(np.ceil(len(audio) / model_audio_length))):
            st = idx * model_audio_length
            audios.append(audio[st : min(st + model_audio_length, len(audio))])
        audios[-1] = np.pad(
            audios[-1],
            (0, model_audio_length - len(audios[-1])),
            mode="constant",
        )
        ...
        #     "src_wav": torch.zeros(model_audio_length).repeat(0),
        #     "recv_wav": torch.from_numpy(audio).unsqueeze(0),
        # }
        batch = {
            "rgb": np.repeat(
                transform(image=rgb)["image"][np.newaxis, :, :],
                batch_size,
                axis=0,
            ),
            "src_wav": torch.zeros((batch_size, model_audio_length)),
            "recv_wav": torch.from_numpy(audios),
        }
        ...
        sf.write(args.output_audio, pred.cpu().numpy(), 16000)
